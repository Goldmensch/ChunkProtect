...
public class UntrustSub extends ChunkProtectSubCommand {
  ...
  public UntrustSub(ChunkProtectPlugin chunkProtectPlugin, ChunkProtectCommand command) {
    super(ExecutorLevel.PLAYER, chunkProtectPlugin.getPermission("untrust"), chunkProtectPlugin,
        command);
  }
  ...
  public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
    ...
    if (target == null) {
      getMessenger().send(sender, "player.notFound",
    ...
    if ((args.length == 2) && "this".equalsIgnoreCase(args[1])) {
      ...
      if (claimedChunk.getTrustedPlayer().remove(target.getUniqueId())) {
        getMessenger().send(sender, "player.untrusted",
            Replacement.create(IMessenger.PLAYER_LITERAL, target.getName()));
      } else {
        getMessenger().send(sender, "player.not.trusted",
            Replacement.create(IMessenger.PLAYER_LITERAL, target.getName()));
      }
      return true;
    }
    ...
    if (holder.getTrustedAllChunks().remove(target.getUniqueId())) {
      getMessenger().send(player, "player.untrustedOnAll",
          Replacement.create(IMessenger.PLAYER_LITERAL, target.getName()));
    } else {
      getMessenger().send(player, "player.not.trustedOnAll",
          Replacement.create(IMessenger.PLAYER_LITERAL, target.getName()));
    }
    return true;
  }
