...
public class TrustSub extends ChunkProtectSubCommand {
  ...
  public TrustSub(ChunkProtectPlugin chunkProtectPlugin, ChunkProtectCommand command) {
    super(ExecutorLevel.PLAYER, chunkProtectPlugin.getPermission("trust"), chunkProtectPlugin,
        command);
  }
  ...
  public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
    ...
    if (target == null) {
      getMessenger().send(sender, "player.offline",
          Replacement.create(IMessenger.PLAYER_LITERAL, args[0]));
      return true;
    }

    if ((args.length == 2) && "this".equalsIgnoreCase(args[1])) {
      ClaimableChunk claimableChunk = Chunks.chunkFromPlayer(player, getDataService());
      if (Chunks.isClaimedAndHolder(claimableChunk, player, getChunkProtect())) {
        return true;
      }
      ClaimedChunk claimedChunk = claimableChunk.getChunk();

      if (claimedChunk.getTrustedPlayer().add(target.getUniqueId())) {
        getMessenger().send(sender, "player.trusted",
            Replacement.create(IMessenger.PLAYER_LITERAL, target.getName()));
      } else {
        getMessenger().send(sender, "player.already.trusted",
            Replacement.create(IMessenger.PLAYER_LITERAL, target.getName()));
      }
      return true;
    }
    ...
    if (holder.getTrustedAllChunks().add(target.getUniqueId())) {
      getMessenger().send(sender, "player.trustedOnAll",
          Replacement.create(IMessenger.PLAYER_LITERAL, target.getName()));
    } else {
      getMessenger().send(sender, "player.already.trustedOnAll",
          Replacement.create(IMessenger.PLAYER_LITERAL, target.getName()));
    }
    return true;
  }
