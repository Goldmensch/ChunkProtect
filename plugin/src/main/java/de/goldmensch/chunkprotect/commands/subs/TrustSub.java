...
public class TrustSub extends ChunkProtectSubCommand {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (holder.getTrustedAllChunks().add(target.getUniqueId())) {
            getMessenger().send(sender, "player.trustedOnAll", Replacement.create(IMessenger.PLAYER_LITERAL, target.getName()));
        } else {
            getMessenger().send(sender, "player.already.trustedOnAll", Replacement.create(IMessenger.PLAYER_LITERAL, target.getName()));
        }
        return true;
    }
