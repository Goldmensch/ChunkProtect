...
public class InfoSub extends ChunkProtectSubCommand {
  ...
  public InfoSub(ChunkProtectPlugin chunkProtectPlugin, ChunkProtectCommand command) {
    super(ExecutorLevel.PLAYER, chunkProtectPlugin.getPermission("info"), chunkProtectPlugin,
        command);
  }
      ...
      if (player != null) {
        CompletableFuture.supplyAsync(() -> {
          ...
          sender.sendMessage(MessageBuilder.builder()
              ...
              .appendLine(getMessenger()
                  .prepare("player.info.uuid", Replacement.create("uuid", holder.getUuid()
                      .toString())))
              .appendLine(getMessenger()
                  .prepare("player.info.claimamount",
                      Replacement.create("amount", String.valueOf(holder.getClaimAmount()))))
              .appendLine(getMessenger()
                  .prepare("player.info.onAllTrusted",
                      Replacement.create("names", infoValue.getTrusted()
                          .toString())))
              .build());
        });
      } else {
    ...
    if (claimableChunk.isClaimed()) {
      ...
      CompletableFuture.supplyAsync(() ->
          ...
          sender.sendMessage(MessageBuilder.builder()
              ...
              .appendLine(getMessenger()
                  .prepare("chunk.info.holder", Replacement.create("holder", chunk.getHolder()
                      .getName())))
              .appendLine(getMessenger()
                  .prepare("chunk.info.trusted",
                      Replacement.create("playerlist", chunkHolders.toString())))
              .build())
      );
    } else {
