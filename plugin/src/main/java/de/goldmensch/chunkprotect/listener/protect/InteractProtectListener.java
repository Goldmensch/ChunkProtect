...
public class InteractProtectListener extends EntityListeners {
  ...
  public void handleBucketFill(PlayerBucketFillEvent event) {
    Chunks.getChunk(event.getBlock().getChunk(), dataService).ifClaimedOr(chunk -> {
      if (protectionFile.getOther().getBucketFill().isClaimed() && forbidden(event.getPlayer(),
          chunk)) {
        event.setCancelled(true);
      }
    }, () -> {
      if (protectionFile.getOther().getBucketFill().isUnclaimed() && hasNoBypass(
          event.getPlayer())) {
  ...
  public void handleBucketEmpty(PlayerBucketEmptyEvent event) {
    Chunks.getChunk(event.getBlock().getChunk(), dataService).ifClaimedOr(chunk -> {
      if (protectionFile.getOther().getBucketEmpty().isClaimed() && forbidden(event.getPlayer(),
          chunk)) {
        event.setCancelled(true);
      }
    }, () -> {
      if (protectionFile.getOther().getBucketEmpty().isUnclaimed() && hasNoBypass(
          event.getPlayer())) {
        sendYouCantDoThat(event.getPlayer());
        event.setCancelled(true);
      }
  ...
  public void handlePlayerInteractEntity(PlayerInteractEntityEvent event) {
    Chunks.getChunk(event.getRightClicked().getChunk(), dataService).ifClaimedOr(chunk -> {
      if (entitiesConfiguration.getProtection(event.getRightClicked().getType())
          .getPlayerInteract()
          .isClaimed() && forbidden(event.getPlayer(), chunk)) {
        event.setCancelled(true);
      }
    }, () -> {
      if (entitiesConfiguration.getProtection(event.getRightClicked().getType()).getPlayerInteract()
          .isUnclaimed()
  ...
  public void handlePlayerInteract(PlayerInteractEvent event) {
    if (event.getClickedBlock() != null
      ...
      if (handleBlock(protectionFile.getBlock().getPlayerInteract(), event.getPlayer(),
